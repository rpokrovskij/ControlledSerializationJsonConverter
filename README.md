...
1) to easy configure json formatting for certain types. 
2) to avoid crashes by walking through circular references (setting recursion depth).
3) to limit serialization to certain types or unique (by reference) objects.
...
1) **formatters** - `Dictionary<Type, Func<object, string>>` - register serailize `Func<object, string>`" function for a type `Type`. Serialize function can be null, then .ToString() will be used;
2) **recursionDepth** - number of steps in the object's graph after which serialization will be stoped (avoiding circular references and infinite serialization process); default `recursionDepth` is 4;
3) **supremeTypes list** - it is a way to switch serialization execution between your custom code and MS standard serializers. In a standard situation it is a list of CLR standard simple types (like `int`, `datetime`, etc) which will be serialized without going deep into theirs properties therefore "supreme types" are leafs in serialized object's graph;  you can add custom types to this list (both reference and scalar types); To process `supreme type` serializing process exit ControlledSerializationJsonConverter code and returns to `JavaScriptSerializer` (where you can hook it), therefore you should understand `JavaScriptSerializer` very good; parameter accessible only in protected constructor; more easy way to customize serialization is using of "Formatters" (see bellow); difference is that using `supremeTypes list` you can decide dynamically "is this object a leaf or or a brunch".
4) **supportedTypes list and ignoreNotSupported flag** - control serialization of specific types. In simplest configuration it should contain at least one item (type of the object you are going to serialize), otherwise `ControlledSerializationJsonConverter` will be not hooked by `JavaScriptSerializer` process and will not get a chance to start a work. This is the another MS `JavaScriptSerializer` specific that can't be avoided. Once  converter was hooked, it not return serialization execution to parent process till it meet *suprem types* (leafs). If `ignoreNotSupported` setuped to `true` then all *not supreme types* you are interested in serialization should be in this list otherwice they will be ignored. In that case  list of types usually will be constructed with `Assembly.GetTypes()` call; 
5) **ignore duplicates** - when this flag is setuped, then reference types objects will be tracked in internal cache by theirs references and one and the same object will be not serialized twice;
6) **ignoreScriptIgnoreAttribute** - default value is `false` that means all *ScriptIgnoreAttribute* attributes will be ignored; with `ControlledSerializationJsonConverter` there are no sense to use *ScriptIgnoreAttribute* widely. 
